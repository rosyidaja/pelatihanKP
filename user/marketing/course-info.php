...
$query = mysqli_query($koneksi, "SELECT p.peserta_jenis, pel.jns_pelatihan_nama, t.id_jns_pelatihan, p.peserta_instansi_nama, j.jadwal_sesi, u.user_nama, t.id_peserta, t.id_jadwal, t.approve, t.reg_no, t.lokasi, t.tools, t.status_pay, t.trainer FROM m_peserta P INNER JOIN(m_jadwal j INNER JOIN(m_jns_pelatihan pel INNER JOIN(t_sertifikasi t INNER JOIN m_user u ON t.id_marketing=u.user_id)ON pel.jns_pelatihan_kode=t.id_jns_pelatihan)ON j.jadwal_id=t.id_jadwal) ON p.peserta_id=t.id_peserta WHERE t.id_marketing='$mID' GROUP BY p.peserta_instansi_nama ORDER BY p.peserta_id ASC");
...
<div class="header">
  ...
  <p><i class="fa fa-info-circle"></i> Untuk konfirmasi registrasi, Silahkan pilih metode pembayaran terlebih dahulu.</p>
</div>
    ...
    <table id="tbmarket" class="table table-striped table-hover table-sm text-center">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="align-middle">
            <input type="text" class="form-control apr-disabled<?php echo $no;?>" id="lokasi<?php echo $regno; ?>" placeholder="<?php if ($data["lokasi"] == "") { echo "Masukkan Lokasi"; } else { echo $lokasi; } ?>">
